...
class HyperDip(MainNetInterface, nn.Module):
    def __init__(self, num_input_channels=2, num_output_channels=3,
        ...
        self._layer_weight_tensors = [
            module.weight for module in self.model.modules() if hasattr(module, "weight")]
        self._layer_bias_vectors = [
            module.bias for module in self.model.modules() if hasattr(module, "weight")]
        self._internal_params = nn.ParameterList(
            self._layer_weight_tensors + self._layer_bias_vectors)
            ...
            if isinstance(module, nn.Sequential):
                for i in range(len(module)):
                    if isinstance(module[i], nn.BatchNorm2d):
                        ...
                        module[i] = BatchNormLayer(
                            num_features=num_features, momentum=momentum, affine=affine, track_running_stats=track_running_stats)
